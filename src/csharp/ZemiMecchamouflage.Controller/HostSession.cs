...
public sealed class HostSession
{
    ...
    // The second pass (F5) may only run after a real first pass (F1) has completed at
    ...
    public Task<HostCommandResult> RunPaintAsync(bool previewOnly, bool unpreviewOnly, CancellationToken cancellationToken = default) =>
        RunPaintAsync(PaintKind.Standard, previewOnly, unpreviewOnly, cancellationToken: cancellationToken);
    ...
    public Task<HostCommandResult> RunImagePaintAsync(bool previewOnly, bool unpreviewOnly, CancellationToken cancellationToken = default) =>
        RunPaintAsync(PaintKind.Image, previewOnly, unpreviewOnly, cancellationToken: cancellationToken);
        ...
        int naturalColorBatchSeed = 0)
    {
        ...
        {
            const string secondPassGated = "Second pass: run the first pass (start hotkey) at least once first.";
            Log.Warn(secondPassGated);
            return new HostCommandResult(false, secondPassGated);
        }
        ...
        try
        {
            ...
            var startedMessage = startedMessageOverride ??
                (previewOnly ? "Preview: started." : (unpreviewOnly ? "UnPreview: started." : (secondPass ? "Second pass: started." : (kind == PaintKind.Image ? "Image Paint: started." : "Paint: started."))));
            ...
            if (response.Success)
            {
                ...
                if (!previewOnly && !unpreviewOnly && !secondPass)
    ...
    /// <summary>
    /// The second pass reuses the exact mesh-first paint route, but with its own brush size
    /// and color compression tolerance, and it must never re-touch a region the user configured
    /// as Fill. Any region not explicitly set to Paint is forced to Skip for this pass only;
    /// the persisted settings are never modified.
    /// </summary>
    private static AppSettings BuildSecondPassSettings(AppSettings source)
    {
        var clone = Clone(source);
        var paint = clone.Paint;
        paint.BrushSizeTexels = paint.SecondPassBrushSizeTexels;
        paint.ColorCompressionTolerance = paint.SecondPassColorCompressionTolerance;
        if (paint.FrontRegionMode != RegionMode.Paint)
            paint.FrontRegionMode = RegionMode.Skip;
        if (paint.SideRegionMode != RegionMode.Paint)
            paint.SideRegionMode = RegionMode.Skip;
        if (paint.BackRegionMode != RegionMode.Paint)
            paint.BackRegionMode = RegionMode.Skip;
        return SettingsStore.Clamp(clone);
    }
